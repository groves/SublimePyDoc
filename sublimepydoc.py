...
class OpenPythonModuleUnderCursorCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        webbrowser.open_new('http://docs.python.org/library/%s.html' % module)


class OpenPythonModuleCommand(sublime_plugin.TextCommand):
    '''Opens one of the modules in the modules list below. Run sublimepydoc.printmodules() to generate the list'''
    def run(self, edit):
        def x(result):
            if result != -1:
                webbrowser.open_new('http://docs.python.org/library/%s.html' % modules[result])
